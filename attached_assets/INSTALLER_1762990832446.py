...
class M3UMatrixInstaller:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("M3U MATRIX PRO - Complete Installer")
        self.root.geometry("800x600")
        self.root.configure(bg="#121212")
        
        self.setup_done = False
        self.install_path = Path.home() / "M3U_MATRIX_PRO"
    ...
    def create_ui(self):
        ...
        tk.Label(header, text="🎬 M3U MATRIX PRO", font=("Impact", 32), 
                fg="gold", bg="#8e44ad").pack(pady=20)
        tk.Label(header, text="Complete All-in-One IPTV Manager", 
                font=("Arial", 14), fg="white", bg="#8e44ad").pack()
        ...
        progress_frame = tk.LabelFrame(main_frame, text="Installation Progress", 
                                      fg="gold", bg="#1e1e1e", font=("Arial", 12, "bold"),
                                      padx=20, pady=20)
        progress_frame.pack(fill=tk.X, pady=(0, 20))
        
        self.progress = ttk.Progressbar(progress_frame, mode='determinate', length=700)
        ...
        # Log Section
        log_frame = tk.LabelFrame(main_frame, text="Installation Log", 
                                 fg="gold", bg="#1e1e1e", font=("Arial", 12, "bold"),
                                 padx=20, pady=20)
        log_frame.pack(fill=tk.BOTH, expand=True)
        
        self.log_text = tk.Text(log_frame, height=15, bg="#111111", fg="#00ff00",
        ...
        self.install_btn = tk.Button(button_frame, text="🚀 START COMPLETE INSTALLATION", 
                                   bg="#27ae60", fg="white", font=("Arial", 14, "bold"),
                                   command=self.start_installation, width=30, height=2)
        self.install_btn.pack(pady=10)
        
        tk.Button(button_frame, text="❌ Cancel", bg="#e74c3c", fg="white",
                 command=self.root.quit, width=15).pack()
        
        self.log("Welcome to M3U MATRIX PRO Installer!")
        self.log(f"Installation folder: {self.install_path}")
        self.log("Click START to begin automated setup...")
    ...
    def install_dependencies(self):
        ...
        for package in packages:
            try:
                self.update_progress(20, f"Installing {package}...")
    ...
    def create_directory_structure(self):
        ...
        try:
            self.update_progress(40, "Creating directory structure...")
            ...
            for folder in folders:
                (self.install_path / folder).mkdir(exist_ok=True)
    ...
    def create_main_application(self):
        """Create the main M3U Matrix application file"""
        try:
            self.update_progress(60, "Creating main application...")
            
            app_content = '''# M3U_MATRIX_PRO.py
import tkinter as tk
...
"""
    ...
    def inline_edit(self, iid, col_idx):
        ...
        self.editor = tk.Entry(self.tv, bg="#333", fg="#fff", font=("Segoe UI", 10), relief=tk.FLAT, highlightthickness=2, 
...
'''
            ...
            app_file = self.install_path / "M3U_MATRIX_PRO.py"
            with open(app_file, 'w', encoding='utf-8') as f:
                f.write(app_content)
            
            self.log("✓ Main application created successfully")
            return True
        except Exception as e:
            self.log(f"✗ Failed to create main application: {e}")
            return False
    
    def create_launcher_scripts(self):
        """Create launcher scripts for Windows and other platforms"""
        try:
            self.update_progress(80, "Creating launcher scripts...")
...
pause
'''
            bat_file = self.install_path / "RUN_M3U_MATRIX.bat"
            with open(bat_file, 'w') as f:
                f.write(bat_content)
            
            # Windows VBS file (silent launcher)
            vbs_content = '''Set WshShell = CreateObject("WScript.Shell")
WshShell.Run "python """ & CreateObject("Scripting.FileSystemObject").GetAbsolutePathName(".") & "\M3U_MATRIX_PRO.py""", 0, False
'''
            vbs_file = self.install_path / "RUN_M3U_MATRIX_SILENT.vbs"
...
'''
            sh_file = self.install_path / "RUN_M3U_MATRIX.sh"
            ...
            self.log(f"✗ Failed to create launcher scripts: {e}")
            return False
    
    def create_readme(self):
        """Create comprehensive README file"""
        try:
            self.update_progress(90, "Creating documentation...")
            
            readme_content = '''# M3U MATRIX PRO - Complete IPTV Management Suite

## 🚀 Quick Start
1. Double-click `RUN_M3U_MATRIX.bat` (Windows) or `RUN_M3U_MATRIX.sh` (Linux/Mac)
2. Application starts automatically with all features ready
